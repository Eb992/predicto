...
from .kelly_criterion import Bet, KellyCriterion
...
class ValueBetFinder:
    ...
    def find_value_bets(
        self, predictions: List[Dict], odds_data: pd.DataFrame = None
    ) -> List[ValueBet]:
        """
        ...
        for match_pred in predictions:
            ...
            for market, market_preds in match_pred.get("predictions", {}).items():
                for pred in market_preds:
                    # Ottieni quote
                    odds = self._get_odds(match_pred, market, pred["selection"], odds_data)
    ...
    def _get_odds(
        self, match: Dict, market: str, selection: str, odds_data: pd.DataFrame = None
    ) -> Optional[float]:
    ...
    def filter_by_confidence(
        self, value_bets: List[ValueBet], min_confidence: str = "medium"
    ) -> List[ValueBet]:
        """Filtra value bets per livello di confidenza."""
        confidence_order = {"low": 0, "medium": 1, "high": 2}
        min_level = confidence_order.get(min_confidence, 1)

        return [vb for vb in value_bets if confidence_order.get(vb.confidence, 0) >= min_level]

    def get_daily_picks(
        self, value_bets: List[ValueBet], max_bets: int = 5, max_exposure: float = 0.15
    ) -> List[ValueBet]:
        """
