...
class PlayerStatsModel(BasePredictionModel):
    ...
    def __init__(
        ...
        super().__init__(
            model_name=f"Player_{stat_type}", min_accuracy=min_accuracy, random_state=random_state
        )
    ...
    def _create_model(self):
        ...
        if self.use_regression:
            return HistGradientBoostingRegressor(
                max_depth=6, learning_rate=0.1, max_iter=150, random_state=self.random_state
            )
        else:
    ...
    def get_player_features(self) -> List[str]:
        ...
        stat_specific = {
            ...
            "fouls_committed": ["player_fouls_per_90", "referee_avg_fouls", "match_importance"],
            "yellow_cards": ["player_yellows_per_90", "referee_avg_yellows", "player_fouls_per_90"],
        }

        return base_features + stat_specific.get(self.stat_type, [])

    def predict_player_stat(self, X: pd.DataFrame, player_name: str = None) -> pd.DataFrame:
        """
    ...
    @classmethod
    def create_multi_stat_models(
        cls, stats: List[str] = None, **kwargs
    ) -> Dict[str, "PlayerStatsModel"]:
        """
        ...
        return {stat: cls(stat_type=stat, **kwargs) for stat in stats}
