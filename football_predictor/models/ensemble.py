...
class EnsemblePredictor:
            ...
            if name in y_dict:
                ...
                if y is not None and len(y.dropna()) > 50:
                    ...
                    try:
                        ...
                        logger.info(
                            f"Modello {name} addestrato - Accuracy: {model.metrics. get('accuracy_mean', 0):.2%}"
                        )
    ...
    def get_combined_prediction(
        self, X: pd.DataFrame, market: str
    ...
    def evaluate_ensemble(self, X: pd.DataFrame, y_true: Dict[str, pd.Series]) -> Dict[str, float]:
...
class StackedEnsemble(EnsemblePredictor):
    ...
    def __init__(
        self, base_models: Dict[str, BasePredictionModel] = None, meta_learner: BaseEstimator = None
    ):
    ...
    def fit(self, X: pd.DataFrame, y: pd.Series, y_dict: Dict[str, pd.Series] = None):
        """
