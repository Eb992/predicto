...
class FBrefScraper(BaseScraper):
    ...
    def fetch_player_match_stats(
        self, stat_type: str = "summary", match_id: Optional[str] = None
    ) -> pd.DataFrame:
        """
        ...
        for league, scraper in self.scrapers.items():
            try:
                stats = scraper.read_player_match_stats(stat_type=stat_type, match_id=match_id)
                all_data.append(stats)
            except Exception as e:
...
def create_scraper(
    ...
    **kwargs,
    ...
    return scrapers[source](
        leagues=leagues, seasons=seasons, use_tor=use_tor, tor_port=tor_port, **kwargs
    )
