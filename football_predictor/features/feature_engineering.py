...
class FeatureEngineer:
    ...
    def compute_team_form(self, team:  str, before_date: datetime, 
                          n_games: int = 5) -> Dict[str, float]: 
        """Calcola la forma recente della squadra."""
        team_matches = self._matches[
            ((self._matches['home_team'] == team) | 
             (self._matches['away_team'] == team)) &
    ...
    def compute_h2h_features(self, home_team: str, away_team: str, 
                             before_date: datetime, n_games:  int = 10) -> Dict[str, float]:
        """Calcola statistiche head-to-head."""
        h2h_matches = self._matches[
    ...
    def compute_player_impact(self, team:  str, season: int) -> Dict[str, float]: 
        ...
        try:
            team_players = self._player_stats_df[
                self._player_stats_df. index.get_level_values('team') == team
            ] if 'team' in self._player_stats_df.index.names else pd.DataFrame()
            ...
            return {
                ...
                'team_top_scorer_goals': team_players. get('goals', pd.Series([0])).max() if 'goals' in team_players.columns else 0,
                'team_key_player_influence': 0.5
            }
        except Exception: 
            return {
    ...
    def compute_all_features(self, home_team: str, away_team: str, 
                             match_date: datetime, referee: Optional[str] = None,
        ...
        # Differenze chiave
        features['form_diff'] = (features['home_form_points_per_game'] - 
                                  features['away_form_points_per_game'])
        features['attack_diff'] = (features['home_form_goals_scored_avg'] - 
                                    features['away_form_goals_scored_avg'])
        features['defense_diff'] = (features['away_form_goals_conceded_avg'] - 
                                     features['home_form_goals_conceded_avg'])
