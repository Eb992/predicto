...
from ..betting.kelly_criterion import BankrollManager, Bet, KellyCriterion
...
class Backtester:
    ...
    def __init__(
        self, initial_bankroll: float = 1000.0, kelly_fraction: float = 0.25, min_edge: float = 0.05
    ):
        ...
        for i, test_date in enumerate(tqdm(unique_dates[min_train_matches:], desc="Backtesting")):
            ...
            for bet in day_bets:
                ...
                self.bankroll_manager.current_bankroll = (
                    self.bankroll_manager.kelly.current_bankroll
                )
    ...
    def _merge_odds(self, data: pd.DataFrame, odds_data: pd.DataFrame) -> pd.DataFrame:
        ...
        return pd.merge(data, odds_data, on=["date", "home_team", "away_team"], how="left")
    ...
    def _get_odds(self, row: pd.Series, market: str, selection: str) -> Optional[float]:
        ...
        odds_mapping = {
            "1X2_Result": {"H": "odds_home_win", "D": "odds_draw", "A": "odds_away_win"},
            "BTTS": {
                "True": "odds_btts_yes",
                "False": "odds_btts_no",
                True: "odds_btts_yes",
