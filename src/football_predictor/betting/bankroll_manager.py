...
class BankrollManager:
    ...
    def __init__(
        ...
        self.kelly = KellyCriterion(bankroll=initial_bankroll, kelly_fraction=kelly_fraction)
    ...
    def should_stop_trading(self) -> Tuple[bool, str]:
        ...
        if profit_pct >= self.profit_target_pct:
            return (
                True,
                f"PROFIT_TARGET: Profitto {profit_pct:. 1%} >= {self.profit_target_pct:.1%}",
            )
        ...
        if daily_loss_pct >= self.daily_loss_limit_pct:
            return (
                True,
                f"DAILY_LIMIT: Perdita giornaliera {daily_loss_pct:.1%} >= {self.daily_loss_limit_pct:.1%}",
            )
