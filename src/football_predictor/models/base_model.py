...
class BasePredictionModel(ABC):
    ...
    def __init__(self, model_name: str, min_accuracy: float = 0.60, random_state: int = 42):
    ...
    def fit(self, X: pd.DataFrame, y: pd.Series) -> "BasePredictionModel":
        ...
        calibrated_model = CalibratedClassifierCV(self.model, cv=3, method="isotonic")
    ...
    def _create_model(self) -> BaseEstimator:
        ...
        models = [
            ...
            (
                "gb",
                HistGradientBoostingClassifier(
                    max_depth=8, learning_rate=0.1, max_iter=200, random_state=self.random_state
                ),
            ),
            ("lr", LogisticRegression(C=1.0, max_iter=1000, random_state=self.random_state)),
        ]
...
class ExactScoreModel(BasePredictionModel):
    ...
    def _create_model(self) -> BaseEstimator:
        ...
        return HistGradientBoostingClassifier(
            max_depth=12, learning_rate=0.05, max_iter=500, random_state=self.random_state
        )
