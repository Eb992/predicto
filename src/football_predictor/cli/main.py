...
from ..betting.kelly_criterion import BankrollManager, KellyCriterion
...
def backtest(ctx, start_date, end_date, bankroll, kelly_fraction, leagues, output):
    ...
    with Progress() as progress:
        ...
        try:
            ...
            progress.update(
                task, advance=20, description="[cyan]Scaricando statistiche giocatori..."
            )
    ...
    backtester = Backtester(
        initial_bankroll=bankroll, kelly_fraction=kelly_fraction, min_edge=config.betting.min_edge
    )

    # Aggiungi modelli
    backtester.add_model("1X2_Result", MatchResultModel(min_accuracy=0.60))
    backtester.add_model(
        "Exact_Score", ExactScoreModel(min_accuracy=0.15)
    )  # Più basso per exact score
    ...
    results = backtester.run_backtest(
        data=processed_data, odds_data=odds_data, train_window_days=365, test_window_days=30
    )
...
@click.option(
    ...
    default=["all"],
    help="Mercati da predire",
)
@click.option(
    "--min-probability", "-p", default=0.55, help="Probabilità minima per mostrare predizione"
)
...
def predict(ctx, days, leagues, markets, min_probability, output):
    ...
    with Progress() as progress:
        ...
        # Genera predizioni
        predictions = generate_predictions(fixtures, models, feature_engineer, min_probability)
...
def value_bets(ctx, bankroll, kelly_fraction, predictions_file, odds_file):
    ...
    console.print(
        Panel.fit("[bold yellow]💰 Value Bets Finder[/bold yellow]", border_style="yellow")
    )
    ...
    kelly = KellyCriterion(
        bankroll=bankroll, kelly_fraction=kelly_fraction, min_edge=config.betting.min_edge
    )
...
def stats(ctx, file):
    ...
    console.print(
        Panel.fit("[bold magenta]📈 Statistiche Scommesse[/bold magenta]", border_style="magenta")
    )
...
def prepare_backtest_data(
    schedule: pd.DataFrame, feature_engineer: FeatureEngineer, odds_data: pd.DataFrame
) -> pd.DataFrame:
    ...
    for idx, row in schedule.iterrows():
        try:
            ...
            features["match_id"] = (
                f"{row.get('date')}_{row.get('home_team')}_{row.get('away_team')}"
            )

            # Target variables
            home_goals = row.get(
                "home_goals",
                row.get("score", "0-0").split("-")[0] if isinstance(row.get("score"), str) else 0,
            )
            away_goals = row.get(
                "away_goals",
                row.get("score", "0-0").split("-")[1] if isinstance(row.get("score"), str) else 0,
            )
...
def prepare_training_data(
    historical_data: pd.DataFrame, feature_engineer: FeatureEngineer
) -> pd.DataFrame:
    """Prepara i dati di training."""
    return prepare_backtest_data(historical_data, feature_engineer, None)


def train_models(train_data: pd.DataFrame, markets: list, config: Config) -> dict:
    ...
    if "1x2" in markets:
        ...
        if len(y) > 100:
            ...
            models["1x2"] = model
            console.print(
                f"[green]✓ Modello 1X2 - Accuracy: {model.metrics.get('accuracy_mean', 0):.2%}[/green]"
            )
    ...
    if "exact" in markets:
        ...
        if len(y) > 100:
            ...
            models["exact"] = model
            console.print(
                f"[green]✓ Modello Exact Score - Accuracy: {model.metrics.get('accuracy_mean', 0):.2%}[/green]"
            )
    ...
    if "btts" in markets:
        ...
        if len(y) > 100:
            ...
            models["btts"] = model
            console.print(
                f"[green]✓ Modello BTTS - Accuracy: {model.metrics.get('accuracy_mean', 0):.2%}[/green]"
            )
    ...
    if "over_under" in markets:
        ...
        if len(y) > 100:
            ...
            console.print(
                f"[green]✓ Modello Over/Under 2.5 - Accuracy: {model.metrics.get('accuracy_mean', 0):.2%}[/green]"
            )
...
def get_upcoming_fixtures(scraper: FBrefScraper, days: int) -> pd.DataFrame:
    ...
    upcoming = schedule[(schedule["date"] >= today) & (schedule["date"] <= end_date)]

    return upcoming


def generate_predictions(
    fixtures: pd.DataFrame, models: dict, feature_engineer: FeatureEngineer, min_probability: float
) -> list:
...
def find_value_bets(predictions: list, odds_data: pd.DataFrame, kelly: KellyCriterion) -> list:
...
def get_actual_odds(match: dict, market: str, selection: str, odds_data: pd.DataFrame) -> float:
...
def display_backtest_results(results):
    ...
    if results.model_accuracies:
        console.print("\n")
        model_table = Table(
            title="🎯 Accuracy Modelli", show_header=True, header_style="bold magenta"
        )
    ...
    for pred in predictions[:20]:  # Limita a 20 partite
        ...
        for market, market_preds in pred.get("predictions", {}).items():
            ...
            for p in market_preds[:3]:
                ...
                console.print(
                    f"    • {p['selection']}: [{conf_color}]{p['probability']:.2%}[/{conf_color}]"
                )
